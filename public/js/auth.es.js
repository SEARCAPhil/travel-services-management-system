...
(async () => {
  const AdalConfig = await import('../../config-js/adal')
  const ApiConfig = await import('../../config-js/api') 
...
const loginOnPremiseServer = (data) => { 
  return new Promise((resolve, reject) => {
       XHR.then(res => {
        ...
        const payload = {
          url:`${ApiConfig.default.url}/authentication/confirmation`,
          method:'POST',
          body: JSON.stringify({
              data,
          }),
...
const getImage = (token) => {
  ...
  console.log("%cTravel Services Management System : You are trying to view the code in console","color: grey")
